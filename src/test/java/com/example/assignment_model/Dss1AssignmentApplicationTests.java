...
class Dss1AssignmentApplicationTests {
		...
		for (int i = 0; i < numClients; i++) {
			...
			if (r < numGets) {
				executorService.submit(() -> {
					...
					ResponseEntity<Audio> response = restTemplate.exchange(baseUrl + "audio/" + artistName,
							HttpMethod.GET, entity, Audio.class);
					System.out.println("Response for artist " + artistName + " : "
							+ Objects.requireNonNull(response.getBody()).getTrackTitle());
				});
			} else {
				executorService.submit(() -> {
					RestTemplate restTemplate = new RestTemplate();
					...
					ResponseEntity<String> response = restTemplate.exchange(baseUrl + "newAudio", HttpMethod.POST,
							entity, String.class);
	...
	public void testGetAllAudioItems() {
		...
		headers.setContentType(MediaType.APPLICATION_JSON);
		HttpEntity<String> entity = new HttpEntity<>(headers);
		ResponseEntity<String> response = restTemplate.exchange(baseUrl + "audios", HttpMethod.GET, entity,
				String.class);
	...
	public void testCreateAudioItem() {
		...
		headers.setContentType(MediaType.APPLICATION_JSON);
		HttpEntity<Audio> entity = new HttpEntity<>(newAudio, headers);
		ResponseEntity<String> response = restTemplate.exchange(baseUrl + "newAudio", HttpMethod.POST, entity,
				String.class);
		System.out.println("Response for creating audio item: " + response.getBody());
	}
